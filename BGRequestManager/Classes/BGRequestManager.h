...
#ifdef DEBUG
#define BGAppLog(s, ... ) NSLog( @"[%@ in line %d] ===============>%@", [[NSString stringWithUTF8String:__FILE__] lastPathComponent], __LINE__, [NSString stringWithFormat:(s), ##__VA_ARGS__] )
#else
#define BGAppLog(s, ... )
#endif
 ...
 */
typedef void (^BGDownloadProgress)(int64_t bytesRead,
int64_t totalBytesRead);

typedef BGDownloadProgress BGGetProgress;
typedef BGDownloadProgress BGPostProgress;
 ...
 */
typedef void (^BGUploadProgress)(int64_t bytesWritten,
int64_t totalBytesWritten);

typedef NS_ENUM(NSUInteger, BGResponseType) {
    kBGResponseTypeJSON = 1, // 默认
    kBGResponseTypeXML  = 2, // XML
    // 特殊情况下，一转换服务器就无法识别的，默认会尝试转换成JSON，若失败则需要自己去转换
    kBGResponseTypeData = 3
};

typedef NS_ENUM(NSUInteger, BGRequestType) {
    kBGRequestTypeJSON = 1, // 默认
    kBGRequestTypePlainText  = 2 // 普通text/html
};

typedef NS_ENUM(NSInteger, BGNetworkStatus) {
    kBGNetworkStatusUnknown          = -1,//未知网络
    kBGNetworkStatusNotReachable     = 0,//网络无连接
    kBGNetworkStatusReachableViaWWAN = 1,//2，3，4G网络
    kBGNetworkStatusReachableViaWiFi = 2,//WIFI网络
};
...
// 且处理，请转换成对应的子类类型
typedef NSURLSessionTask BGURLSessionTask;
typedef void(^BGResponseSuccess)(id response);
typedef void(^BGResponseFail)(NSError *error);
 ...
 *  @note 这里只提供公共api
 */
@interface BGRequestManager : NSObject
...
/**
 *
 *	取消某个请求。如果是要取消某个请求，最好是引用接口所返回来的BGURLSessionTask对象，
